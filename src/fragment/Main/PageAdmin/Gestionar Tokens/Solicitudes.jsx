...
const Solicitudes = (props) => {
  ...
  const tokensFiltrados = tokens.filter((token) => {
    ...
    const tokenBuscado =
      ...
      token.latitud.toLowerCase().includes(terminoBuscado.toLowerCase()) ||
      token.longitud.toLowerCase().includes(terminoBuscado.toLowerCase()) ||
    ...
    if (buscarEstado !== "Seleccionar") {
      if (buscarEstado === "ACTIVO") {
        return tokenBuscado && token.estado;
      }
  ...
  const usuariosFiltrados = usuarios.filter((user) => {
    ...
    const usuarioBuscado =
      String(user.identificador)
        .toLowerCase()
        .includes(terminoBuscado.toLowerCase()) ||
      user.latitud.toLowerCase().includes(terminoBuscado.toLowerCase()) ||
      user.longitud.toLowerCase().includes(terminoBuscado.toLowerCase()) ||
      (terminoBuscado.toLowerCase() === estadoActivo.toLowerCase() &&
        user.estado) ||
      (terminoBuscado.toLowerCase() === estadoInactivo.toLowerCase() &&
        !user.estado);
    if (buscarEstado !== "Seleccionar") {
      if (buscarEstado === "ACTIVO") {
        return usuarioBuscado && user.estado;
      }
      if (buscarEstado === "INACTIVO") {
        return usuarioBuscado && !user.estado;
      }
